...
def _get_env_var_or_die(k):
    if k not in os.environ or not os.environ[k]:
        print("Expected %s in OS Environment varialbles." % k)
        sys.exit(1)
    return os.environ[k]


KUBERNETES_BRANCH = _get_env_var_or_die('K8SJS_KUBERNETES_BRANCH')
SPEC_VERSION = _get_env_var_or_die('K8SJS_SPEC_VERSION')
...
def strip_tags_from_operation_id(operation, _):
    ...
    for t in operation['tags']:
        operation_id = operation_id.replace(t, '')
    operation['operationId'] = operation_id


def convert_tags_to_camel_case(operation, _): 
    new_tags = []
    for t in operation['tags']:
        new_tags.append(_to_camel_case(t))
    if new_tags:
        operation['tags'] = new_tags


def process_swagger(spec):
    apply_func_to_spec_operations(spec, convert_tags_to_camel_case)
